#!/usr/bin/env node
// walkthrough JSONの検証。swen128/dotfilesのvalidate.tsを依存ゼロのNodeへ移植したもの。
// usage: node validate.mjs <walkthrough.json> [--check-files]
// exit: 0=成功 / 1=スキーマ・ファイルエラー / 2=usageエラー
...
if (positional.length !== 1) {
  console.error("usage: validate.mjs <walkthrough.json> [--check-files]");
  process.exit(2);
}

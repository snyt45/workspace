...
|------|------|------|
| `,,` | n | ファイル検索 (frecency順) |
| `,r` | n | grep検索 (plain/regex/fuzzy切替: Shift+Tab) |
| `,rw` | n | カーソル下の単語でgrep |
...
|------|------|------|
| `Tab` | i | マルチ選択 |
| `C-q` | i | 選択をquickfixに送信 |
| `S-Tab` | i | grepモード切替 (plain → regex → fuzzy) |
| `C-Up` | i | クエリ履歴 |
...
| キー | モード | 説明 |
|------|------|------|
| `,o` | n | 最近使ったファイル (カレントディレクトリのみ) — snacks |
| `,b` | n | バッファ一覧 — snacks |
| `,gs` | n | git status (off: working tree / on: `base...HEAD`) — snacks |
...
| `gy` | n | 型定義ジャンプ |
| `gr` | n | 参照一覧 |
| `K` | n | ホバー (型情報表示、2回押しでウィンドウに入れる) |
| `gl` | n | diagnosticをフロート表示 |
| `,rn` | n | リネーム |
| `,ca` | n | コードアクション |
| `,cs` | n | ソースアクション (import整理等) |
| `,ll` | n | LSP再起動 |

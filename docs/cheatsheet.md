...
| `,wo` | n | 開く/切り替え統合picker（ロード済みセッション + 未ロードJSON。snacks時は `<C-d>`／一覧では `d` で選択項目をJSONごと直接削除） |
| `,wt` | n | noteフロートをトグル（カーソル下のステップ、なければアクティブステップ。表示中に別ステップ上で押すと切替） |
...
| `,px` | n | 未提出コメント全部をpiへ提出（回答はwalkthrough JSONとして `.walkthroughs/` に保存され `,wo` で開ける） |
...
> フロートにフォーカス（`,w<CR>`）して `e` で編集・`d` で削除（入力UIはコメント追加時と同じ）。直接編集は `,we` でフロートを開かず1キー。
> **回答スレッドへの返信**: 回答walkthrough（thread付きステップ）のフロートで `r` → 返信モーダル。回答は同じJSONの該当スレッドに追記され `,wR` で読める。

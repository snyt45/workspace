...
| `Alt + /` | タイル表示 |
| `Alt + h / j / k / l` | フォーカス移動 |
| `Alt + H / J / K / L` | ウィンドウ入れ替え |
| `Alt + -` / `Alt + =` | ウィンドウサイズ変更 |
| `Alt + Shift + 0` | 均等配置 |
| `Alt + Shift + Enter` | フルスクリーン |
| `Alt + 1...9 A...Z` | workspace切り替え |
| `Alt + Shift + 1...9 A...Z` | ウィンドウをそのworkspaceへ送る |
| `Alt + Shift + Tab` | workspaceを別モニターに移動 |
| `Alt + Enter` | モニター切り替え |
| `Alt + Shift + ;` | サービスモード |
| `ESC` | 設定リロード(サービスモード中) |
| `f` | フローティング切り替え(サービスモード中) |
...
|------|------|
| `,e` | エクスプローラー開閉 |
| `l` / `<CR>` | ディレクトリに入る / ファイルを開く |
| `h` | ディレクトリを閉じる |
| `<BS>` | 親ディレクトリへ |
| `a` / `d` / `r` | 新規作成 / 削除 / リネーム |
| `c` / `m` / `p` | コピー / 移動 / ペースト |
| `H` / `I` | 隠しファイル / gitignoreファイル トグル |
| `]g` / `[g` | 次/前のgit変更 |
| `]d` / `[d` | 次/前の診断 |
| `<leader>/` | 配下をgrep |
...
| `[c` | 前の変更箇所へ |
| `,gb` | blame表示 |
| `,go` | カーソル位置をGitHubで開く (commit固定permalink、ビジュアル選択で範囲リンク) |
...
| `mm` | ズームトグル (カレントを縦横最大化 / もう一度で等分に戻す) |

 ...
 *
 * Ported from amosblomqvist/learn visual-tools as a plain GLOBAL pi extension
 * (was: interactive-subagents hook). Every pi session — including herdr
 * subagents, which are full pi sessions — now has these tools. Modules are
 * isolated per process: only one maker per process, so the module-level
 * session state is naturally isolated from any parallel maker.
 *
 * Rendering shells out to a globally installed @mermaid-js/mermaid-cli
 * (`mmdc`, npm i -g) with a puppeteer config pointing at an installed Chrome,
...
import {
  ...
  writeBody,
  writeFileSync,
} from "./visual-tools-common.ts"

const TOOL_DIR = dirname(fileURLToPath(import.meta.url))
const EXTENSION_DIR = dirname(TOOL_DIR)
// ponytail: global install (`npm i -g @mermaid-js/mermaid-cli`), resolved via PATH
const MMDC_BIN = "mmdc"
const GROUP = "mermaid"
